...
def find(i):
    ...
    while tag:
        try:
            ...
            "/html/body/form/div[5]/div/div[2]/div/div/div/div[1]/div/div/div[2]/input[1]").send_keys('YumingYuan')
            tag = False
        except:
            time.sleep(1)
    tag = True
    ...
    while tag:
        try:
            browser.find_element(by=By.XPATH, value=
            '/html/body/form/div[5]/div/div[2]/div/div/div/div[1]/div/div/div[2]/input[2]').send_keys('21fSaupn')
            tag = False
        except:
            time.sleep(1)
    tag = True
...
if __name__ == '__main__':
    ...
    seq_list = numpy.linspace(1000,nrows-1000, nrows-1000, dtype=int)
    tolist = seq_list.tolist()
    for i in range(4120,nrows):
        find(i)
    # pool = Pool(1)
